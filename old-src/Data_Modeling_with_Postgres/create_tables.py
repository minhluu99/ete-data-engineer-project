...
def create_database():
    ...
    conn = psycopg2.connect("host=127.0.0.1 dbname=minh user=minh password=password")
    conn.set_session(autocommit=True)
    cur = conn.cursor()
    
    # create sparkify database with UTF8 encoding
    cur.execute("DROP DATABASE IF EXISTS dwh_minh")
    cur.execute("CREATE DATABASE dwh_minh WITH ENCODING 'utf8' TEMPLATE template0")
